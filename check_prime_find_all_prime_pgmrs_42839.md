...
int solution(string numbers) {
    ...
    for (int i = 2; i <= stoi_numbers; i++) {
        ...
        // if i has not been checked and i is not an element of numbers, check all i's multiples not prime
  ...
  - ```c++
    ...
    bool checkNumber(int num_to_check, string numbers) {
        ...
        for (int i = 0; i < str_num_to_check.size(); i++) {
            ...
            // When comparing signed with unsigned, the compiler converts the signed value to unsigned. index is signed, numbers.size() is unsigned. when index equals -1, it becomes the largest vlaue of unsigned 
